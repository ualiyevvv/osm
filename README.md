...
```
### Пример ответа
Расстояния как массив массивов, представляющих матрицу, в порядке соответсвующим входным данным. `distances[i][j]` дает расстояние пути от `i-го` источника до `j-го` пункта назначения. Все значения указаны в метрах. Расстояние между одной и той же координатой всегда равно `0`
```json
{
  "distances": [
